...
const ResumeManagement = () => {
    ...
    const handleToggleSelect = (resumeNo) => {
        setSelected((prev) => (prev.includes(resumeNo) ? prev.filter((id) => id !== resumeNo) : [...prev, resumeNo]));
    };
    ...
    const handleDeleteSelected = () => {
        ...
        axios
            .post('/api/resume/deleteSelectedResume', selected)
            .then(() => {
    ...
    const handleStatusChange = (resumeNo, newStatus) => {
        axios
            ...
            .then(() => {
                setTempList((prev) =>
                    prev.map((item) => (item.resume_no === resumeNo ? { ...item, status_yn: newStatus } : item))
                );
            })
            .catch((err) => {
                ...
                <div className="info-section-content-box">
                    <div className="info-header">
                        ...
                        <p className="info-warning">
                            삭제할 경우 복구가 어려우며, JobFolio 이용자에게 해당 항목이 즉시 비노출됩니다. 삭제 시
                            신중히 선택 바랍니다.
                        </p>
                                ...
                                {currentTemplates.map((template) => (
                                    ...
                                    >
                                        ...
                                        <div className="resume-info-box">
                                            <p>
                                                <strong>이력서 번호:</strong> {template.resume_no}
                                            </p>
                                            <p>
                                            ...
                                            <p>
                                                <strong>작성일:</strong>{' '}
                                                {template.create_date ? template.create_date.slice(0, 16) : '날짜 없음'}
                                            </p>
                                            <p>
                                                <strong>작성자:</strong> {template.user_name}
                                            </p>
                                            ...
                                            <div className="status-select-container">
                                                <Select
                                                    ...
                                                    onChange={(e) =>
                                                        handleStatusChange(template.resume_no, e.target.value)
                                                    }
                                                >
                                                    <MenuItem value="N">
                                                        <VisibilityIcon /> 노출
