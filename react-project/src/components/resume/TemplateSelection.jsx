    ...
    try {
        ...
        const templatesWithHtml = await Promise.all(
            res.map(async (template) => {
                const filePath = `http://localhost:80${template.file_pypath.replace(/^.*?resume_output/, '/resumes').replace(/\\/g, '/')}`; // 실제 서버 경로에 맞게 수정
                try {
                    ...
                    return { ...template, html: `<p>템플릿을 불러오는 데 실패했습니다.</p>` };
                }
            })
        );
