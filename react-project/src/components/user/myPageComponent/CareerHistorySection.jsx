...
const CareerHistorySection = React.memo(({ userNo, careerHistoryList, onListChange }) => {
    ...
    const saveAndUpdateForm = async () => {
        ...
        try {
            ...
            if (editingCareerNo) {
                response = await axios.put(`/api/myPage/${userNo}/careerhistory/${editingCareerNo}`, dataToSend); // URL 변경
                alert('경력이 수정되었습니다.');
                ...
                response = await axios.post(`/api/myPage/${userNo}/careerhistory`, dataToSend); // URL 변경
                alert('경력이 저장되었습니다.');
