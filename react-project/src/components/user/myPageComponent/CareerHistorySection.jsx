...
const CareerHistorySection = React.memo(({ userNo, careerHistoryList, onListChange }) => {
  ...
  const deleteItemClick = async (careerNoToDelete) => {
    ...
    if (isConfirm) {
      ...
      try {
        ...
        if (onListChange) {
          const updatedCareerHistoryList = careerHistoryList.filter((career) => career.career_no !== careerNoToDelete);
  ...
  const saveAndUpdateForm = async () => {
    ...
    try {
      ...
      if (editingCareerNo) {
        response = await axios.put(`/api/myPage/${userNo}/careers/${editingCareerNo}`, dataToSend); // URL 변경
        alert('경력이 수정되었습니다.');
        ...
        response = await axios.post(`/api/myPage/${userNo}/careers`, dataToSend); // URL 변경
        alert('경력이 저장되었습니다.');
