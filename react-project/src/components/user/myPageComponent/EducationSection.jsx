...
const EducationSection = React.memo(({ userNo, educationList, onListChange }) => {
    ...
    // 학력 최대 개수 설정!!!!
    const MAX_EDUCATION_COUNT = 5;
    // 추가 버튼 활성화 여부
    const isAddButtonDisabled = educationList.length >= MAX_EDUCATION_COUNT;
    ...
    const addEducation = () => {
        // 학력 개수 설정
        if (educationList.length >= MAX_EDUCATION_COUNT) {
            alert(`학력은 최대 ${MAX_EDUCATION_COUNT}개까지만 추가할 수 있습니다.`);
            return;
        }
    ...
    const modifiedItemClick = async (eduNoToEdit) => {
        ...
        const modifyEdu = educationList.find((edu) => edu.edu_no === eduNoToEdit);
        if (modifyEdu) {
            setCurrentFormEdu(modifyEdu);
            // 찾은 edu_no 를 수정 항목에 상태 설정
            setEditingEduNo(eduNoToEdit);
            // 폼 열기
            setShowAddForm(true); // 수정과 추가가 같은 폼을 사용해야함
            // DatePicker 상태 설정
            setEnrollDate(modifyEdu.enroll_date ? new Date(modifyEdu.enroll_date) : null);
            setGradDate(modifyEdu.grad_date ? new Date(modifyEdu.grad_date) : null);
            setErrors({}); // 폼 닫을 때 에러 상태 초기화
            setErrorMessage(''); // 메시지 초기화
        }
    };
    ...
    // 데이터 저장 및 수정 AXIOS
    const saveAndUpdateForm = async (currentFormEdu) => {
        // 에러 상태 초기화
        setErrorMessage(''); // 메시지 초기화
    ...
    return (
        <div className="careerInfoSection">
            <div className="careerInfoHeader">
                ...
                <p className={`plusFunction ${isAddButtonDisabled ? 'disable-button' : ''}`} onClick={addEducation}>
                            ...
                            <div key={edu.edu_no} className="careerItemDisplay">
                                ...
                                <div className="edu-display-header edu-line-item">
                                    ...
                                    <span className="edu-period">
                                        ...
                                        {edu.edu_status && (
                                            <span className="edu.edu-status-display">({edu.edu_status})</span>
                                        )}
                                    </span>
                                </div>
            ...
            {(showAddForm || editingEduNo !== null) && (
                        ...
                        <div className="educationInputArea">
                            ...
                            <div className="formCol">
                                <DatePicker
                                    ...
                                    readOnly={
                                        currentFormEdu.edu_status === '재학' || currentFormEdu.edu_status === '휴학'
                                    }
                                    minDate={enrollDate}
                                    popperPlacement="bottom-start"
                ...
                > */}
                    ...
                    <div className="formButtons">
                        ...
                        <button onClick={() => saveAndUpdateForm(currentFormEdu)}>저장</button>
                    </div>
                </div>
            )}
        </div>
